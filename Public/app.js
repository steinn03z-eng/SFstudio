(() => {
  ...
  function renderWidgets(){
    ...
    // The preview is local, but an already-open OBS overlay receives the saved
    // state through Socket.IO. Keep the debounce tiny so every control feels
    // live there as well, without firing one request for every slider pixel.
    const scheduleVoiceWidgetSave=()=>{clearTimeout(voiceWidgetSaveTimer);voiceWidgetSaveTimer=setTimeout(async()=>{try{const result=await api('/api/voice-list/settings',{method:'PUT',body:JSON.stringify(s)});settings.voiceList=merge(settings.voiceList,result.voiceList||s);}catch(e){console.warn('voice widget autosave',e);}},80);};
