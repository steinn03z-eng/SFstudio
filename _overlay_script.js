...
function currentViewSettingsKey(){
    ...
    function overlayChatNameColor(item){
      const p = settings.personal || {};
      const platform = String(item?.platform || 'tiktok').toLowerCase();
      const custom = /^#[0-9a-f]{6}$/i.test(String(p.nameCustomColor || '')) ? p.nameCustomColor : '';
      if (p.nameColorMode === 'custom' && custom) return custom;
      if (platform === 'twitch') {
        if (p.twitchNameColor === 'white') return '#ffffff';
        if (p.twitchNameColor === 'custom' && custom) return custom;
        return '#c7a2ff';
      }
      if (p.tiktokNameColor === 'real') return '#fe6f92';
      if (p.tiktokNameColor === 'custom' && custom) return custom;
      return '#ffffff';
    }

    function overlayChatFrameClass(item){
      const p = settings.personal || {};
      if (isSupporterProfile(item) && p.highlightSupporters !== false) return 'avatar-frame-gold';
    ...
    function overlayChatBubbleClass(){
      const mode = String(settings.personal?.bubbleFrame || 'platform');
      if (mode === 'none') return 'bubble-frame-none';
      if (mode === 'role') return 'bubble-frame-role';
      return 'bubble-frame-platform';
    }

    function overlayChatStyleVars(item, kind){
      const p = settings.personal || {};
      const platform = String(item?.platform || 'tiktok').toLowerCase();
      const accent = platform === 'twitch' ? '#9146ff' : '#fe2c55';
      const textColor = p.textColor === 'auto' || !p.textColor ? '#e8ecf4' : p.textColor;
      const font = kind === 'event' ? fontFamily(p.overlayEventFont || p.font) : kind === 'gift' ? fontFamily(p.overlayGiftFont || p.font) : fontFamily(p.font);
      return `--row-accent:${accent};--name-color:${overlayChatNameColor(item)};--message-color:${textColor};--bubble-radius:${Number(p.bubbleRadius ?? 12)}px;--avatar-border-width:${Number(p.avatarBorderWidth ?? 2)}px;--row-gap:${Number(p.rowGap ?? 5)}px;--message-padding:${Number(p.messagePadding ?? 7)}px 9px;--chat-font:${font};`;
    }

    function overlayChatItemHtml(item, kind){
      const p = settings.personal || {};
      const platform = String(item?.platform || 'tiktok').toLowerCase();
      const userName = String(item?.displayName || item?.user || item?.username || item?.uniqueId || 'Usuario').trim() || 'Usuario';
      const identity = normalizeUsername(item?.uniqueId || item?.username || item?.user || userName || 'user');
      ...
      const isSupporter = isSupporterProfile(item) && p.highlightSupporters !== false && supporterHighlightEnabled(platform);
      const rawText = isGift ? '' : getRenderedMessage(item);
      const textScale = p.chatAdjustMessages === true ? autoMessageScale(rawText) : 1;
      const textColor = resolveChatTextColor(isGift || isSupporter ? 'yellow' : (platform === 'twitch' ? p.twitchNameColor : p.textColor));
      const nameTextColor = overlayChatNameColor(item);
      const textContrast = effectContrastColor(nameTextColor);
      const textShadow = effectShadow(p.messageEffect, textContrast);
      ...
      const rowKey = buildOverlayDedupKey(kind, item) || `${kind}:${platform}:${Date.now()}`;
      const topBadges = activityBadgeMarkup(item);
      const metaBadges = badgeChips(item.badges, platform);
      const bubbleClass = overlayChatBubbleClass();
      const activitySize = kind === 'event' ? (p.eventsPanelSize || 'normal') : kind === 'gift' ? (p.giftsPanelSize || 'normal') : (p.chatHorizontalMode || 'normal');
      const activityShape = kind === 'event' ? (p.eventsOverlayShape || 'normal') : kind === 'gift' ? (p.giftsOverlayShape || 'normal') : (p.chatOverlayShape || 'normal');
      const activitySide = kind === 'event' ? (p.eventsOverlayCardSide || 'center') : kind === 'gift' ? (p.giftsOverlayCardSide || 'center') : (p.chatOverlayCardSide || 'center');
      const activityFrame = kind === 'event' ? p.eventsCardFrame !== false : kind === 'gift' ? p.giftsCardFrame !== false : true;
      ...
      const giftAmount = item.amount == null || item.amount === '' ? 1 : item.amount;
      const messageHtml = isGift
        ? `<div class="row-message ${bubbleClass} gift-message-bubble"><div class="gift-media gift-media-real">${giftImage ? `<img src="${esc(giftImage)}" alt="${esc(giftName)}" loading="lazy" onerror="this.remove()">` : `<span class="gift-fallback">🎁</span>`}<span>${esc(giftName)}</span><strong>×${esc(giftAmount)}</strong></div></div>`
        : (rawText ? `<div class="row-message ${bubbleClass}">${rawText}</div>` : '');
      const supportClass = isSupporter ? 'supporter-gold' : '';
      const accent = platform === 'twitch' ? '#9146ff' : '#fe2c55';
      const frame = overlayChatFrameClass(item);
      return `<article class="stream-row ${kind} ${platform} ${isGift ? 'gift-row' : ''} chat-theme-${esc(theme)} chat-anim-${esc(animation)} ${kind}-size-${esc(activitySize)} ${kind}-shape-${esc(activityShape)} ${kind}-side-${esc(activitySide)} ${activityFrame ? '' : 'no-frame'} ${supportClass} ${p.chatAdjustMessages !== false ? 'chat-adjust' : 'chat-no-adjust'}" data-stream-key="${esc(rowKey)}" style="${overlayChatStyleVars(item, kind)}--entry-text-scale:${textScale};--entry-text-color:${esc(textColor || '#e8ecf4')};--entry-text-shadow:${esc(textShadow)};--name-text-shadow:${esc(nameShadow)};--name-stroke:${esc(nameStroke)};--item-accent:${accent};">
        <div class="chat-avatar ${frame} size-${p.avatarSize || 'md'}">${avatarHtml}</div>
        <div class="row-body">
          <div class="row-top">
            <strong class="name-size-${p.nameSize || 'md'} weight-${p.nameWeight || '800'}" style="color:${esc(nameTextColor)};text-shadow:${esc(nameShadow)};-webkit-text-stroke:${esc(nameStroke)}">${esc(userName)}</strong>
            ${p.showBadges !== false ? topBadges : ''}
            ${showPlatform ? `<span class="platform-pill ${platform}">${platform === 'twitch' ? 'TW' : 'TT'}</span>` : ''}
            ${showTime ? `<time>${timeLabel(item.timestamp)}</time>` : ''}
          </div>
          ${messageHtml}
          ${metaBadges ? `<div class="row-meta">${metaBadges}</div>` : ''}
        </div>
      </article>`;
    }
    ...
    function itemHtml(item, kind){
      ...
      const style = kind === 'event' ? String(settings.personal.eventStyle || 'chat') : kind === 'gift' ? String(settings.personal.giftStyle || 'chat') : 'chat';
      if (kind === 'event' && style === 'chat') return overlayChatItemHtml(item, 'event');
      if (kind === 'gift' && style === 'chat') return overlayChatItemHtml(item, 'gift');
      ...
      const bubbleFrame = isChat ? (settings.personal.bubbleFrame === 'role' ? 'frame-role' : 'frame-platform') : 'frame-platform';
