...
function voiceBotSummaryHtml(){
  ...
  return chips
    ...
    function saveActivityBadges(){ /* LIVE badges intentionally remain in memory only; persistent follow/donor state comes from the server. */ }
...
function resolveGiftActivityVoiceAssignment(item){
      ...
      // Siempre reevalúa contra las reglas activas actuales. Esto permite
      // que una regla creada/activada después de que el usuario obtuvo la
      // insignia también tome efecto inmediatamente.
      const rule = findMatchingVoiceRule(syntheticGiftItem);
      if (!rule || !rule.id || !activeRuleIds.has(String(rule.id))) {
        // Si la insignia actual no tiene una regla activa, el usuario vuelve
        // a la voz global. También limpiamos cualquier asignación de regalo
        // anterior para impedir que se reutilice una voz obsoleta.
        if (voiceBot.giftByUser?.[key]) {
          delete voiceBot.giftByUser[key];
          saveVoiceBot();
        }
...
function findMatchingVoiceRule(item){
    ...
    function registerVoiceTriggerForItem(item){
      ...
      if (!rule) {
        // Si acaba de llegar un regalo sin una regla activa, la insignia
        // reemplazó a la anterior y cualquier voz ligada al regalo anterior
        // debe desaparecer inmediatamente. El usuario usará la voz global.
        const type = normalizeTypeName(item?.type || item?.group || "");
        const isGift = type.includes("gift") || Boolean(item?.gift || item?.giftName || item?.giftAlt || item?.giftId || item?.giftImage);
        if (isGift) {
          const staleKey = voiceRuleItemKey(item);
          if (staleKey && voiceBot.giftByUser?.[staleKey]) {
            delete voiceBot.giftByUser[staleKey];
            saveVoiceBot();
          }
          const staleEntry = voiceActivityEntry(item);
          if (staleEntry?.voice && String(staleEntry.voice.kind || "").toLowerCase() === "gift") {
            delete staleEntry.voice;
          }
        }
        return null;
      }
...
function currentViewSettingsKey(){
    ...
    function activityChatRow(item, kind){
      ...
      const avatarClass = isSupporterProfile(item) && pp.highlightSupporters !== false
        ? 'avatar-frame-gold'
        : (pp.avatarFrame === 'none' ? 'avatar-frame-none' : pp.avatarFrame === 'ring' ? 'avatar-frame-ring' : pp.avatarFrame === 'role' ? 'avatar-frame-role' : 'avatar-frame-platform');
    ...
    function activityStreamRow(item, kind){
      ...
      return `<article class="activity-preview activity-real overlayItem activity-stream-row ${typeClass} ${typeClass}-${esc(highlight)} ${typeClass}-layout-${esc(layout)} ${typeClass}-direction-${esc(direction)} ${typeClass}-mode-${esc(mode)} ${typeClass}-size-${esc(size)} ${typeClass}-shape-${esc(shape)} ${typeClass}-side-${esc(side)} ${frame ? '' : 'no-frame'}" data-feed-key="${esc(feedKey)}" style="--activity-accent:${accent};--row-gap:${Number(pp.rowGap ?? 5)}px;font-family:${font}">
        <div class="activity-user-avatar ${isSupporterProfile(item) && pp.highlightSupporters !== false ? 'avatar-frame-gold' : (pp.avatarFrame === 'none' ? 'avatar-frame-none' : pp.avatarFrame === 'ring' ? 'avatar-frame-ring' : pp.avatarFrame === 'role' ? 'avatar-frame-role' : 'avatar-frame-platform')} size-${pp.avatarSize || 'md'}">${avatarHtml}</div>
    ...
    function itemHtml(item, kind){
      ...
      if (kind === 'chat') {
        ...
        const bubbleClass = pp.bubbleFrame === 'none' ? 'bubble-frame-none' : pp.bubbleFrame === 'role' ? 'bubble-frame-role' : 'bubble-frame-platform';
        const adjustClass = pp.chatAdjustMessages !== false ? 'chat-adjust' : 'chat-no-adjust';
        const avatarClass = isSupporterProfile(item) && pp.highlightSupporters !== false ? 'avatar-frame-gold' : (pp.avatarFrame === 'none' ? 'avatar-frame-none' : pp.avatarFrame === 'ring' ? 'avatar-frame-ring' : pp.avatarFrame === 'role' ? 'avatar-frame-role' : 'avatar-frame-platform');
        ...
        const platformPill = pp.showPlatformPill !== false ? `<span class="platform-pill ${platform}">${platform === 'twitch' ? 'TW' : 'TT'}</span>` : '';
        const time = pp.showTimestamps !== false ? `<time>${timeLabel(item.timestamp)}</time>` : '';
        const rowClasses = `stream-row overlayItem ${platform} chat-theme-${pp.chatTheme || 'cloud'} ${animationClass} ${adjustClass}${isSupporterProfile(item) ? ' supporter-gold' : ''}`;
    ...
    function pushEvent(data){ const item = normalizeOverlayActivity(data,'event'); registerActivityBadges(item); rememberSupporter(item); if(String(item.type).toLowerCase() === 'gift' || String(item.group).toLowerCase() === 'gift'){ pushGift(item); return; } if(shouldSkipOverlayDuplicate('event', item)) return; registerVoiceTriggerForItem(item); state.events.unshift(item); if(state.events.length > 240) state.events.length = 240; state.events = clearByAge(state.events, settings.personal.eventsAutoClear, settings.personal.eventsClearSeconds); render(); }
    function pushGift(data){ registerActivityBadges(data); rememberSupporter(data); const item = normalizeOverlayActivity(data,'gift'); item.group='gift'; item.type=item.type==='share'?'gift':item.type; item.action=item.action||'Regalo'; if(shouldSkipOverlayDuplicate('gift', item)) return; registerVoiceTriggerForItem(item); state.gifts.push(item); if(state.gifts.length > 240) state.gifts.length = 240; state.gifts = clearByAge(state.gifts, settings.personal.giftsAutoClear, settings.personal.giftsClearSeconds); render(); }

    applyPersonalizationVisuals();

    socket.on('liveEnded', (info) => { const p=String(info?.platform||'tiktok').toLowerCase(); state.activityBadges[p]={}; state.supporters[p]={}; state.chat=state.chat.filter(x=>String(x?.platform||'').toLowerCase()!==p); state.events=state.events.filter(x=>String(x?.platform||'').toLowerCase()!==p); state.gifts=state.gifts.filter(x=>String(x?.platform||'').toLowerCase()!==p); render(); });
